...
import com.qlp2p.doctorcar.crop.ImageCropActivity;
...
public class AppriserWodeActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_appriser_wode);
    ...
    private void initView() {
        tvTotal.setText("已评估"+myglobal.user.setPriceCount+"辆");
    ...
    private void requestPermission() {
        try {
            if (ContextCompat.checkSelfPermission(mContext, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(myglobal.mainActivity, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE},
                        REQ_PERMISSION);
                m_bPermissionGrant = false;
                return;
            }
            if (ContextCompat.checkSelfPermission(mContext, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(myglobal.mainActivity, new String[]{Manifest.permission.CAMERA}, REQ_PERMISSION);
                m_bPermissionGrant = false;
                return;
            }
    ...
    public void asyncPutObjectFromLocalFile(String path) {
        ...
        UPLOAD_OBJECT = MyConstants.ossUploadObject + time + myglobal.user.userId + ".jpg";
        // 构造上传请求
    ...
    public void onMessageEvent(MessageEvent event) {
        if (event.isMsgOf(MyConstants.PERMISSION_GRANTED)) {
            ...
            if (!m_bPermissionGrant)
                return;
            openCustomAlbum();
        }else if(event.isMsgOf(MyConstants.APPRAISER_SET_PRICE_SUCCESS)){
            myglobal.user.setPriceCount = (Integer.parseInt(myglobal.user.setPriceCount) + 1) + "";
            tvTotal.setText("已评估"+myglobal.user.setPriceCount+"辆");
        }
    }
    ///////////////////////////  date picker end  ///////////////////////////////
