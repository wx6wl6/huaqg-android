...
import com.qlp2p.doctorcar.data.PjOrderListInfo;
...
public class AgentOrderFragment extends BaseFragment {
    ...
    public static AgentOrderFragment newInstance(int type) {
        Bundle args = new Bundle();
        args.putInt("type", type);
        AgentOrderFragment fragment = new AgentOrderFragment();
    ...
    public void getArrItems() {
        ...
        if (ServerUrl.isNetworkConnected(mContext)) {
            try {
                ...
                postMap(ServerUrl.getAgentOrderList, fields, getArrsHandler);
    ...
    private class ListAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            holder.tvBorrower.setText("主贷人姓名：" + info.lenderName);
            holder.tvCarType.setText("车辆型号：" + info.carTypeName);
            if ("0".equals(info.pjdType))
                holder.tvOrderType.setText("类型：预估价格");
            else
                holder.tvOrderType.setText("类型：申请报告");
    ...
    public void onMessageEvent(MessageEvent event) {
        if(event.isMsgOf(MyConstants.AGENT_SUBMIT_ORDER_SUCCESS)) {
            if(type == 0)
                RefreshData();
        }
    }
